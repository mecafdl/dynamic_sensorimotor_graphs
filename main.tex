...
%\end{figure}
% ---
The NNMF algorithm attempt to minimize the root mean squared residual 
\begin{equation}    
D=\dfrac{1}{\sqrt{N_\text{r} N}}\|\bm{H}^\intercal-\bm{W} \bm{F}\|_F.
\label{eq:residual}
\end{equation}
...
% ---
\begin{table}
    \caption{Sensorimotor modules from \ac{irm}}
    \centering\small
...
From the link densities matrix $\bar{\bm{H}}$ between all modules, we apply \ac{nnmf}. One crucial question is the number of factors $k$ used to approximate the $\bar{\bm{H}}$. We chose $k$ following the elbow method as in \cite{Phalen2020Nonnegativematrix} and obtain a set of $N_\text{f} = 25$ nonnegative factors with a mean squared residual of $D=0.1658$ (equation~\ref{eq:residual}). The factors 
 $\small \bm{F} = [ \bm{f}_1 \cdots \bm{f}_i \cdots \bm{f}_{N_\text{f}}]$ and their corresponding scores $\small\left\lbrace w_i(k)\right\rbrace^{N_\text{f}}_{i=1}$ decompose the link densities in this form:
% ---
\begin{equation}
...
x\end{equation}
% ---
For each window of 10 samples where the \ac{imi} between all sensors is computed, we get one score for each factor $\bm{f}_i$. Such decomposition is shown in Figure~\ref{fig:decomposition} where the link density connection is shown as graph with thickness representative of the connection strength between the modules.
%----- 
\begin{figure*}[h!]
    \centering
    \includegraphics[width=1\linewidth]{fig/factors_decomposition.png}
    \caption{Example of factor decomposition of the link density matrix based on equation~\ref{eq:decomposition}. Sample 2460 corresponds to hands touching as seen is figure~\ref{fig:main_factors}. The decomposition has factor $\bm{f}_{20}$ contributing the most, second factor is the other tactile factor $\bm{f}_{16}$. Only the first 3 most contributing factors are shown.}
    \label{fig:decomposition}
\end{figure*}
%-----
The main factor corresponds to that with the highest score, e.g., the highest contribution in the link density. In the illustrative Fig.~\ref{fig:main_factors}, we show some example motor behaviors that led each of the factors to be the main factor.
