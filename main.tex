...
\newacro{dfc}[DFC]{functional connectivity}
...
Advancements in the exploration of the brain's structural and functional connectivity have enhanced our comprehension of its organization and information processing. However, there is still a need to establish a comparison between these brain connections and the sensorimotor signals of an embodied agent. While investigating the structural connectivity of the sensorimotor signals presents challenges, it is feasible to study their functional connectivity to gain insights into the body structure and the development of behaviors through information acquisition. The fundamental idea is that there are consistent patterns, called \emph{\ac{smc}} \cite{Jacquey2019Sensorimotorcontingenciesas}, linking the sensorimotor signals of a robot to its embodiment. These \ac{smc} patterns can be identified and examined using methods based on information-theory methods. This work delves into the time-varying functional structure of an embodied agent's sensorimotor system, leveraging concepts from information and graph theory. 
% ---
\begin{figure}[!ht]
	\centering
...
According to the literature, \acp{smc} play a role in the acquisition of body knowledge, generalization, and goal-directedness \cite{Jacquey2019Sensorimotorcontingenciesas}. However, despite the extensive body of the literature on sensorimotor representations \cite{Nguyen2021Sensorimotorrepresentationlearning}, there is a lack of understanding regarding the links between sensorimotor regularities and body knowledge. Several works \cite{Schmidt2013Bootstrappingperceptionusing,Lungarella2006Mappinginformationflow,Polani2009Modelsinformationprocessing,Bossomaier2016introductiontransferentropy,Olsson2006unknownsensorsactuators} have used information-theoretic metrics to study these relationships in the sensorimotor system. Furthermore, \textit{touch} has been identified as a crucial sensory modality when examining \acp{smc}. For example, the study conducted by Gama et al. \cite{Gama2021Goaldirectedtactile} explored the utilization of intrinsic motivation and goal-babbling to facilitate the learning self-touch on a simulated humanoid robot equipped with artificially sensitive skin. Self-touch for calibration was also used in \cite{Roncone2014Automatickinematicchain}, where the robot autonomously closes its kinematic chain by touching its own body. Authors in \cite{Marcel2022Learningreachown}, employed a denoising framework with a multimodal variational autoencoder to learn the underlying latent representation of self-touch, which allows the agent to internally reconstruct self-reaching configurations.
...
The study of functional connectivity has shown that the statistical properties of the considered signals and their relationships may vary over time. This concept, known as \emph{\ac{dfc}} has mainly been applied in studying functional networks in the brain. For instance, \ac{dfc} has been utilized to identify states of reduced functional connectivity during epilepsy onsets \cite{Christiaen2020Dynamicfunctionalconnectivity} or to detect potentially abnormal connectivity patterns in brains affected by illness \cite{Zhou2020Earlychildhooddevelopmental}. Recently, \ac{dfc} has also been employed to examine evolving relationships in the sensorimotor signals of simulated infants based on age \cite{Kanazawa2023Openendedmovements}. %In robotics, it is expected that depending on the motion policy driving the robot or the task being executed by it, the sensorimotor connectivity will also change. 
...
\end{equation}
% ---
The joint torque is obtained by computing the difference between activation signals for flexion $ \sigma_{fx} $ and extension $\sigma_{ex}$. These activation signals contribute to the generation of the flexion and extension pulling forces, $ f_{fx}$ and $f_{ex} $ respectivly. The parameters are:
% ---
\begin{itemize}
