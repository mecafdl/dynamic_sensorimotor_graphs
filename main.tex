...
Factorizing the vectorized link densities matrix $\bar{\bm{H}}$ using \ac{nnmf} leads to a set of basis factors  $\bm{F} = [ \bm{f}_1 \cdots \bm{f}_i \cdots \bm{f}_{N_\text{f}}]$ and their corresponding scores $\left\lbrace w_i(k)\right\rbrace^{N_\text{f}}_{i=1}$ that approximate the link densities as
% ---
\begin{equation}
...
\begin{figure*}[h!]
    ...
    \caption{\textbf{Factors' scores, main factors and agent's behavior.} \textsc{Top}: agent's behavior at 8 moments,  samples $[2400,2430,2460,2490,2519,2549,2579,2609]$, the movement is 10 samples long, it corresponds to the time window on which the IMI is computed, and shown in shades of pink from darker/older to lighter/newer. The agent goes from waving the arms to hands touching, two hands touching the torso, right hand touching the torso, and finally back to no-touch. \textsc{Middle}: stacked plot of each factor scores. In shades of blue are represented the factors with high sensitivity to connection with visual modules, resp. red for tactile modules and green for proprioceptive. \textsc{Bottom}: evolution of the main factor, e.g., factor with the highest contribution..}
    \label{fig:factors_time}
\end{figure*}
%-----
Factors represent sensory synergies, their scores represent their contribution to the actual movement of the agent. In Fig.~\ref{fig:factors_time} we observe the evolution of scores and main factor during the agent's movement. 

\myhl{general conclusion here about the factors.}
%%
\section{Discussion}
How does our findings relate to sensorimotor contingencies? In our work, we obtain an agent that is able to autonomously cluster functional modules to create a "patchwork" of sensory clusters that covers all sensory inputs. Then by decomposing the information shared between modules in connection patterns we obtain a set of descriptors of the impact of movement on the sensory experience: I can't touch my torso without flexing my shoulder, I can change my vision on the right by waving my right elbow. By linking proprioception to vision or touch, we obtain patterns that are functionnaly different and,when added together, represent the current sensorimotor interaction: the same way we can touch our mouth with our left arm while waving the right hand in the air. 

The additive property of the factors means that new factors can be discovered and added as the agent's experience new interactions. 

-Aslo developmental properties of the factors, additive, body changing
