...
\title{\LARGE \bf
An analytical framework to infer body features and behaviors from sensorimotor dynamic functional connectivity
}
...
This work introduces an analytical framework, illustrated in Fig.~\ref{fig:general_overview}, to investigate the formation of relationships between the sensorimotor signals of a simulated two-dimensional embodied agent. Specifically, the framework examines the time-varying \acl{fc} between the agent's proprioceptive, tactile, and visual signals. During an initial exploratory phase based on motor babbling, the agent gathers information about its evolving sensorimotor relationships. The framework models these changing relationships as time-varying graphs, serving as a proxy for understanding the formation and evolution of \acp{smc}. To achieve this, we compute the instantaneous pairwise \ac{mi}, capturing the \acl{dfc} that correlates the agent’s information-sharing state with its motion and potential interactions. A key step in the framework involves applying a Bayesian approach to uncover hidden structures within the mutual information-based connectivity. This analysis identifies clusters of highly related signals and their evolving interactions over time. Finally, the framework decomposes these dynamic relationships to extract meaningful patterns (i.e., motifs) in the graphs, which represent distinct information-sharing states. The insights derived from these states and their transitions reveal fundamental aspects of the agent’s body structure and behavior.

% \myhl{This work presents an analytical framework, depicted in Fig.~\ref{fig:general_overview}, to unravel the potential formation of relationships between the sensorimotor signals of a simulated 2-dimensional embodied agent. In particular, the framework looks at the time-varying \acl{fc} between the agent's proprioceptive, tactile, and visual signals. 
% Following an exploratory phase based on motor babbling, the agent gathers information about its evolving sensorimotor relationships. The proposed framework considers these developing relationships as time-varying graphs that serve as a proxy to understand the formation and evolution of \acp{smc}. To achieve this, we compute the instantaneous pairwise \ac{mi}, to capture the \acl{dfc} that correlates the agent’s information-sharing state correlates with its motion and potential interactions. A further step in the framework involves the use of a Bayesian approach to discover structure hidden in the \ac{mi}-based connectivity. The identified structure expresses clusters of highly related signals and their corresponding time-varying interactions with other clusters. Consequently, a final stage in the framework involves decomposing these varying relationships between clusters to identify relevant patterns (i.e. motifs) in the graphs that express distinct information-sharing states. The insights provided by the consideration of these different information states and their transitions reveal essential aspects of the agent’s body structure and behavior.}
...
For a time window of length $T$, the \ac{mi} $I_t(s_x(t);s_y(t))$ between a distinct pair of signals $s_x(t)$ and $s_y(t)$ at time $t$ is computed using the set of signal samples spanning the $\left[t-T,t\right]$ interval\footnote{The selection of a relatively short time window is motivated by the fact that tactile events often occur within a short timescale.}, as illustrated in Fig.~\ref{fig:mi_sliding_window_mi}. We refer to this quantity as the \ac{imi}. 

Extending this concept, the \ac{mi} matrix $\bm{\mathcal{I}}(t)$ at time $t$ is constructed by calculating the \ac{imi} for all pairwise signals within the same time interval. The temporal evolution of this time-varying \ac{mi} matrix $\bm{\mathcal{I}}(t)$ captures the \ac{dfc} between the sensorimotor signals.
% ---
\begin{figure}[!t]
	\begin{center}
		\hspace*{\fill}
