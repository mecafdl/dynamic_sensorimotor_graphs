...
\author{Fernando D\'iaz Ledezma$^{1}$, Valentin Marcel$^{2}$, and Matej Hoffmann$^{2}$% <-this % stops a space
\thanks{*We gratefully acknowledge the funding of this work by the Deutsche Forschungsgemeinschaft through the Gottfried Wilhelm Leibniz Programme (award to Sami Haddadin; grant no. HA7372/3-1).}% <-this % stops a space
...
% Improving the motion policy used to collect sensorimotor signals is an aspect not addressed. The current study used motor babbling. However, better, perhaps goal-directed, or active exploration alternatives might lead to better results that can capture and reproduce particular sensorimotor behaviors. One more possible alternative similar to \cite{Kanazawa2023Openendedmovements} would be to record motion from real infants, map it with the use of musculoskeletal models to obtain proprioceptive and tactile signals, and then apply our analytical framework to discover motor behaviors.
...
% The current implementation of our framework requires collecting and processing information offline. An online implementation of the framework capable of, for example, readapting the number of clusters or factors is still an open question. This takes more relevance as the agent's motor policy changes since new fundamental behaviors (factors from the \ac{nnmf}) could be discovered. Furthermore, as factors are additive, newly discovered factors could take over and replace the older ones.
Our framework currently relies on offline data collection and processing. Developing an online implementation that can dynamically adapt—such as adjusting the number of clusters or factors in real-time—remains an open challenge. This capability becomes particularly relevant as the agent's motor policy evolves, potentially revealing new fundamental behaviors (i.e., factors from the \ac{nnmf}). Additionally, given the additive nature of these factors, newly discovered ones could emerge and gradually replace older ones, reflecting the agent's ongoing adaptation.
...
% \myhl{This work presented a method for the analysis of the \ac{dfc} among the perceptual signals of an embodied agent to discover and characterize motor behaviors. The method depicted in Fig.~\ref{fig:framework_overview} was successfully applied to the collected signals from the embodied agent in Fig.~\ref{fig:extended_dual_arm_robot} to demonstrate that the \ac{imi} can indeed capture the time-varying functional connections between the perceptual variables. The locally changing connectivity was run through an \ac{irm}, which detected $N_\text{c} = 23$ sensorimotor modules and their corresponding \ac{dfc} in the form of the link density matrix $\bm{H}$. Finally, \ac{nnmf} decomposed the information from the \ac{irm} into $N_\text{f} = 23$ additive factors (subgraphs) $\bm{f}_i$ and their corresponding coefficients for each time $w_i(t)$. Ultimately, these factors can be interpreted as the basic motor behaviors in Fig.~\ref{fig:main_factors}.} 
