...
\begin{figure*}[h!]
	...
	% \caption{\textbf{From sensorimotor \acl{dfc} to behavior primitives.}~Motor babbling excites the agent's sensorimotor system. While moving, the \ac{imi} captures the changing sensorimotor \ac{fc}. The \ac{irm} clusters the pairwise \ac{imi} and identifies their associated dynamic relations. Finally, \ac{nnmf} decomposes these relationships into basis factors signifying underlying behavior patterns present in the original data.}
...
%    \includegraphics[width=0.95\textwidth]{fig/irm_results_figure.png}
    ...
    \vspace{5pt}
    % \caption{\textbf{Extracted sensorimotor modules and their \ac{dfc}.} \textsc{Right}: $N_\text{c}=23$ functional modules---clusters---found by the \ac{irm}, each assigned to a different color. We see a great separation of modules by modalities, e.g., proprioceptive, tactile, and visual modules. \textsc{Left}: Correspondence between the agent's receptive fields and their associated module color. \textsc{Bottom}: The matrix $\bm{H}$ from the \ac{irm} captures the \ac{dfc} between the $N_\text{c}$ modules.}
