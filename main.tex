...
Structural connectivity describes the physical links between different components within a network. In neuroscience, for example, this typically refers to the anatomical pathways connecting various brain regions \cite{Park2013StructuralAF}. In contrast, \acf{fc} captures statistical dependencies or temporal correlations between the activities of different network elements. As outlined in \cite{Friston2011FunctionalAE}, \ac{fc} is an information-theoretic measure that quantifies these dependencies by analyzing the probability distributions of observed signals. %Functional connectivity is particularly valuable for analyzing signal relationships, as it captures the brain's dynamic interactions and indirect connections, providing insights into behavior, cognition, and neurological conditions by observing the real time interactions between brain regions
...
In robotics, \ac{fc} is expected to change based on the robot’s motion policy or the task it performs. Capturing and analyzing these evolving patterns using \ac{dfc} could provide a deeper understanding of how an agent’s sensory and motor systems interact over time.
...
We started from the model described in \cite{Mannella2018Knowyourbody} and used in \cite{Marcel2022Learningreachown}, adding population coding to model the sensory receptors. This model consists of a simple planar dual-arm system with six degrees of freedom, featuring three links per arm and a fixed torso, see Fig.~\ref{fig:extended_dual_arm_robot}. The robot is equipped with tactile sensors distributed across its body. 
...
Tactile sensors are randomly distributed along the robot's one-dimensional body and modeled using population coding \cite{Panzeri2010PopulationCoding}. Each sensor is represented by a Gaussian receptive field, whose mean is determined by the sensor's location. To incorporate touch strength, we modulate the activation of each receptive field based on the contact force, adjusting the distance-dependent response accordingly.  
...
\end{equation}
% ---
for a pair $\left({s}_i(t),{s}_j(t)\right)$ involves centering their samples and using either binning, kernel, or nearest neighbor methods to compute their \ac{mi} \cite{WaltersWilliams2009Estimationmutualinformation}.
...
Reiterating, \ac{nnmf} decomposes the sensorimotor \ac{dfc} in $\bm{H}$ into: (1) a set of overlapping patterns $\bm{F}$---i.e., functional subnetworks---that evolve across space and time and (2) corresponding coefficient time series $\bm{W}$ that indicate the contribution of each subnetwork at a given time.
...
\begin{figure*}[t!]
    ...
    \vspace{5pt}
    \caption{\textbf{Extracted sensorimotor modules and their \ac{dfc}.} \textsc{Right}: A total of $N_\text{c}=23$ clusters---representing functional modules---were found by the \ac{irm}, each assigned to a different color. We see a great separation of modules by modalities, e.g., proprioceptive, tactile, and visual modules. \textsc{Left}: Correspondance between the agent's neurons and its correspondant module color. \textsc{Bottom}: The matrix $\bm{H}$ from the \ac{irm} captures the \ac{dfc} between the $N_\text{c}$ modules.}
...
\begin{figure*}[h!]
    ...
    \caption{\textbf{Example of factor decomposition of the link density matrix based on \ac{nnmf}.} At sample 2,460, the agent's hands touch each other, as seen in Fig.~\ref{fig:main_factors}. The decomposition shows that factors $\bm{f}_{20}$, $\bm{f}_{16}$, and $\bm{f}_{14}$ contribute the most to this beavior. %The decomposition shows factor $\bm{f}_{20}$ contributes the most, with second factor is the other tactile factor $\bm{f}_{16}$. Only the first three most contributing factors are shown.
    }
...
\end{figure*}
%-----
The \acl{nnmf} has the property to extract features in the data such that each found factor represents a unique body synergy. In Fig.~\ref{fig:factors_time}, we observe the evolution of scores and main factors during the agent's movement. To showcase another property of the combination of \ac{irm} and \ac{nnmf} for the analysis, consider that while the visual sensors are the most represented in the raw neural signals with 225 neurons (compared to 48 proprioceptive neurons), the contribution of vision is quite reduced as reflected by the scores $\bm{W}$. Our proposed process normalizes the density of neural activation by their functional and informative content to describe the current behavior; as an example, such property would provide a way to weigh sensory prediction errors in sensorimotor learning methods.

%\myhl{general conclusion here about the factors.}
%%

%=============================================================================
\section{Conclusion}
%\myhl{Talk about hierarchical or different implementation of irm/ the fact that find a way to normalize wrt to sensors densities.}
...
From the \acl{imi}, we captured the time-varying functional connections between proprioceptive, tactile, and visual signals, revealing fundamental sensorimotor relationships. 
By applying an \acl{irm}, we identified 23 sensorimotor modules and their evolving connectivity, represented by the link density matrix $\bm{H}$. 
...
The factors can be interpreted as a basis \ac{fc} graph, capturing distinct states of information sharing between the sensorimotor clusters. These factors also encode fundamental movement patterns, representing the agent’s embodied interactions with its environment. They serve as proxies for \acp{smc} that emerge during motor babbling. At any given time, the agent’s observed interaction patterns can be viewed as a weighted combination of these \acp{smc}, governed by the entries of the coefficient matrix $\bm{W}$. By extracting structured representations of an embodied agent’s sensorimotor dynamics, our approach demonstrates the effectiveness of combining information-theoretic measures (\ac{mi}), Bayesian nonparametric models (\ac{irm}), and high-dimensional analysis (\ac{nnmf}) in studying \acp{smc}.
...
\section{Discussion}
How do our findings relate to \acp{smc}? In our work, we obtain an agent that is able to autonomously cluster functional modules to create a ``patchwork'' of sensory clusters that covers all sensory inputs. Then, by decomposing the information shared between modules in connection patterns, we obtain a set of descriptors of the impact of movement on the sensory experience like: ``I can't touch my torso without flexing my shoulder,'' or ``I can generate visual input on the right by waving my right elbow.'' By linking proprioception to vision or touch, we obtain patterns that are functionally different and, when added together, represent the current sensorimotor interaction, just as we can touch our mouth with our left arm while waving the right hand in the air. 


The inputs to the information-theoretic analysis are a result of the agent's movements, but the method looks only at the structure of the multimodal sensory signals. Explicitly adding the motor or muscle commands would be a logical extension. Additionally, \ac{mi} is a symmetrical measure of the relationship between two variables (i.e., time series), and hence, it does not pick up the direction of information flow (potential causality). Alternative directional measures---transfer entropy or Granger causality---could be used.
%Our aim was to introduce the \ac{dfc} within sensory signals to describe the agent's body behavior, thanks to the use of proprioceptive signals we obtain synergies between sensations and efferent copies from the muscles, but not the actual excitatory command. An interesting extension would be to consider a directional information-theoretic measure, such as transfer entropy or Granger causality, to study the impact of neural commands on the muscles and the sensory synergies. This would allow the agent to adjust its control.

We used random motor babbling as the motion policy for collecting sensorimotor signals. In the future, alternative strategies such as goal-directed or active exploration could improve the ability to capture and reproduce specific sensorimotor behaviors. Inspired by \cite{Kanazawa2023Openendedmovements}, one promising approach would be to record motion data from real infants, map it onto musculoskeletal models to extract proprioceptive and tactile signals, and then apply our framework to uncover underlying motor behaviors. 

Additionally, our framework currently operates offline, requiring pre-collected data for analysis; other methods, such as incremental \ac{nnmf} \cite{Bucak2007}, can be implemented to adjust the number of factors as the experience increases. This adaptability becomes particularly important as the agent's motor policy evolves, potentially revealing new fundamental behaviors (i.e., factors from the \acl{nnmf}). Moreover, given the additive nature of these factors, newly discovered ones could gradually replace older ones, reflecting the agent’s ongoing adaptation to its environment. Addressing these limitations in future work could enhance the framework’s applicability, particularly for real-time learning and adaptive sensorimotor behavior discovery.


%\myhl{to add -Aslo developmental properties of the factors, additive, body changing
%- dynamics (we use only one time scale)
%- self vs outside)
...
% ---
% \begin{table}
%     \caption{\textbf{Sensorimotor modules from \ac{irm}}}
%     \centering\small
%     \begin{tabular}{ |p{3cm}||p{3cm}|  }
%         % \hline
%         % \multicolumn{2}{|c|}{Sensorimotor modules from \ac{irm}} \\
%         \hline
...
%         \hline
%     \end{tabular}
%     \label{tab:sensorimotor_modules_from_irm}
% \end{table}
% % ---